...
- (void)jsq_attachmentButtonPressed:(UIButton *)sender
{
    [self.delegate messagesInputToolbar:self didPressAttachmentButton:sender];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if (context == kJSQMessagesInputToolbarKeyValueObservingContext) {
        if (object == self.contentView) {
            ...
            else if ([keyPath isEqualToString:NSStringFromSelector(@selector(attachmentButtonItem))]) {
                
                [self.contentView.attachmentButtonItem removeTarget:self
                                                           action:NULL
                                                 forControlEvents:UIControlEventTouchUpInside];
                
                [self.contentView.attachmentButtonItem addTarget:self
                                                        action:@selector(jsq_attachmentButtonPressed:)
                                              forControlEvents:UIControlEventTouchUpInside];
            }
...
- (void)jsq_addObservers
{
    ...
    [self.contentView addObserver:self
                       forKeyPath:NSStringFromSelector(@selector(attachmentButtonItem))
                          options:0
                          context:kJSQMessagesInputToolbarKeyValueObservingContext];
